...
4. **Outreach Copier & Templates:** Blueprint outreach sequences (cold, warm, referral emails, meeting prep briefs, objection handlers) that draft copy-paste ready copy under 100 words.
5. **Agentic Skill Packaging (`SKILL.md`):** Packaged to be instantly dropped into modern AI agent environments (like Antigravity or Claude Code) as a custom system capability.
...
│   ├── analyze_prospect.py  # Zero-dependency website crawler
│   ├── contact_finder.py    # Non-backtracking team & leadership parser
│   └── lead_scorer.py       # Pure-python BANT & MEDDIC scoring logic
...
You can run these scripts directly in any terminal environment using python 3.x with **zero external dependencies**.
