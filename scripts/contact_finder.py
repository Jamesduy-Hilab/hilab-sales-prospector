...
def extract_card_people(html):
    ...
    people = []
    # Pattern 1: heading (name) followed by paragraph (title), limited tag lookahead to avoid backtracking
    pattern1 = re.compile(
...
def extract_list_people(html):
    ...
    people = []
    # Split html by li tags to avoid catastrophic backtracking on large documents
    li_blocks = re.findall(r'<li[^>]*>(.*?)</li>', html, re.IGNORECASE | re.DOTALL)
    ...
    for block in li_blocks:
        if len(block) > 500:
            continue
        m = li_pattern.search(block)
        if m:
            name = _clean_text(m.group(1))
            title = _clean_text(m.group(2))
            if _is_valid_person(name, title):
                lk_match = re.search(r'https?://(?:www\.)?linkedin\.com/in/[\w-]+/?', block, re.IGNORECASE)
                linkedin = lk_match.group(0) if lk_match else ""
                people.append({"name": name, "title": title, "linkedin": linkedin})
    return people
